...
Built-in MCPs (enabled by default):
- `websearch` (Exa AI) — real-time web search
- `context7` (library docs) — official documentation lookup
- `grep_app` (GitHub code search) — ultra-fast code search across public repos
- `lsp` (local language-server tools) — diagnostics, symbols, references, renames
- `codegraph` (local stdio) — code graph exploration and analysis
- `sequential_thinking` (local stdio) — dynamic reflective problem-solving through thought sequences
- `github` (local stdio) — full GitHub repository management (requires `GITHUB_PERSONAL_ACCESS_TOKEN`)

Remote MCPs (`websearch`, `context7`, `grep_app`) are probed at startup with a 2-second timeout. Unreachable MCPs are automatically disabled so the plugin works offline. Structural search and rewrite is provided by the `ast-grep` skill instead of a built-in MCP.

```json
{ "disabled_mcps": ["websearch", "context7", "grep_app", "lsp", "codegraph", "sequential_thinking", "github"] }
```

**Environment variables for built-in MCPs:**
- `EXA_API_KEY` / `TAVILY_API_KEY` — optional API keys for websearch providers
- `CONTEXT7_API_KEY` — optional API key for Context7
- `GITHUB_PERSONAL_ACCESS_TOKEN` — **required** for the `github` MCP. If absent, the MCP is silently skipped.
