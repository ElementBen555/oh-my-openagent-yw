...
| ---- | ------ | ------------------------------- |
| 1 — Built-in | Injected at runtime by oh-my-openagent (`websearch`, `context7`, `grep_app`, `lsp`, `codegraph`, `sequential_thinking`, `github`) | No |
...
| MCP | Type | Description | Requirements |
| --- | ---- | ----------- | ------------ |
| **websearch** | remote | Real-time web search powered by Exa AI | Optional `EXA_API_KEY` or `TAVILY_API_KEY` |
| **context7** | remote | Official documentation lookup for any library/framework | Optional `CONTEXT7_API_KEY` |
| **grep_app** | remote | Ultra-fast code search across public GitHub repos. Great for finding implementation examples. | None |
| **lsp** | local stdio | Local LSP tools for diagnostics, symbols, references, and renames | Node/Bun runtime |
| **codegraph** | local stdio | Code graph exploration: search callers, callees, impact analysis | `codegraph` binary on PATH or provisioned |
| **sequential_thinking** | local stdio | Dynamic reflective problem-solving through thought sequences | `npx` (Node) |
| **github** | local stdio | Full GitHub repository management: issues, PRs, branches, commits, code search, security scanning | `GITHUB_PERSONAL_ACCESS_TOKEN` env var |

**Offline resilience**: Remote MCPs are probed at startup with a 2-second timeout. Unreachable servers are automatically disabled (`enabled: false`) so the plugin continues to function without network access.
