...
| `GITHUB_PERSONAL_ACCESS_TOKEN` | Enables the built-in `github` MCP (repository/PR/issue management). Without it, the MCP is silently skipped. |
| `EXA_API_KEY` / `TAVILY_API_KEY` | Optional API keys for the `websearch` MCP (Exa/Tavily providers) |
| `CONTEXT7_API_KEY` | Optional API key for the `context7` MCP (library documentation) |
