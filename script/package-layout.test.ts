...
function collectExpectedAssetPaths(): string[] {
	return [...commandRoots.flatMap(collectCommandAssetPaths), ...skillRoots.flatMap(collectSkillAssetPaths)].sort();
}
...
describe("published package layout", () => {
	test("#given vendored LSP MCP package #when inspecting tracked package files #then it is not a git submodule", () => {
		...
		const gitmodules =
			trackedGitmodulesPath && existsSync(gitmodulesPath) ? Bun.file(gitmodulesPath).text() : Promise.resolve("");
	...
	test(
		"#given Codex hook component runtimes #when packing package #then every hook command target ships",
		async () => {
			// given
