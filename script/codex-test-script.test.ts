...
describe("Codex compatibility test script", () => {
	...
	test("runs the vendored LSP package tests after building its package", () => {
		...
		// #then
		expect(testsLspAfterBuild, "test:codex must run the vendored LSP package test suite after building it").toBe(
			true,
		);
	});
	...
	test("builds lsp-daemon before installer tests copy packaged runtimes", () => {
		...
		const installerTestIndex = packageManifest.indexOf(
			"packages/omo-codex/src/install/install-codex-packaged.test.ts",
		);
		const buildsLspDaemonBeforePluginAndInstallerTests =
			lspDaemonBuildIndex >= 0 && pluginBuildIndex > lspDaemonBuildIndex && installerTestIndex > lspDaemonBuildIndex;
