...
describe("LazyCodex publish workflow", () => {
	test("publishes a LazyCodex GitHub release only when the marketplace payload changed", () => {
		...
		const publishAliasDefaultsOn =
			workflow.includes("publish_lazycodex:") &&
		...
		const syncBuildsCodexPlugin =
			...
			workflow.indexOf("bun run --cwd packages/omo-codex/plugin build") <
				workflow.indexOf("bun run script/sync-lazycodex-marketplace.ts");
		const npmPublishBuildsLspDistBeforeMainPackage = buildMainPackageStep.includes(
			"bun run build:lsp-tools-mcp && bun run build",
		);
		...
		const tokenRequirementBeforePublish =
			workflow.indexOf("name: Require LazyCodex sync token") < workflow.indexOf("publish-main:");
		const requiresLazycodexSyncToken =
			workflow.includes("LAZYCODEX_SYNC_TOKEN: ${{ secrets.LAZYCODEX_SYNC_TOKEN }}") &&
		...
		// #then
		expect(stampsCodexPluginMetadata, "LazyCodex plugin metadata must be stamped with the release version").toBe(
			true,
		);
		expect(lazycodexSyncHasNoManualOptOut, "LazyCodex marketplace sync must not expose a manual opt-out input").toBe(
			true,
		);
		...
		expect(
			npmPublishBuildsLspDistBeforeMainPackage,
			"release must build LSP dist before npm packages include it",
		).toBe(true);
		expect(
		...
		expect(exposesChangedOutput, "LazyCodex release state must expose changed and previous-version outputs").toBe(
			true,
		);
		expect(
	...
	test("skips the LazyCodex GitHub release when the previous payload is unchanged", () => {
		...
		expect(releaseStepRequiresChangedOutput, "LazyCodex release step must skip when lazycodex_changed is false").toBe(
			true,
		);
	...
	test("can skip LazyCodex npm alias publishing when npm holds the package name", () => {
		...
		const checkStepIsConditional =
			workflow.includes("id: check-lazycodex") && workflow.includes("if: inputs.publish_lazycodex == true");
		const rebuildsOnlyWhenEnabledOrOtherPackagesNeedPublishing = updateVersionStep.includes(
	...
	test("publishes lazycodex as a Node installer without Bun-backed runtime dependencies", () => {
		...
		const lazycodexStepScopesPublishedFiles = workflow.includes(
			'.files = ["dist/cli", "dist/cli-node", "packages/omo-codex/scripts/install-local.mjs", "packages/omo-codex/scripts/install-dist", "packages/omo-codex/plugin", "packages/omo-codex/plugin/components/start-work-continuation/dist/cli.js", "packages/omo-codex/plugin/components/ulw-loop/dist/cli.js", "packages/omo-codex/plugin/.codex-plugin", "packages/omo-codex/marketplace.json", "packages/omo-codex/lazycodex-repository", "packages/lsp-tools-mcp/package.json", "packages/lsp-tools-mcp/dist", "packages/lsp-daemon/package.json", "packages/lsp-daemon/dist", "packages/git-bash-mcp/dist", "packages/shared-skills"]',
		);
	...
	test("smoke tests the published LazyCodex alias after npm publish", () => {
		...
		// #when
		const smokeRunsAfterPublishBeforeRestore =
			publishIndex >= 0 && smokeIndex > publishIndex && restoreIndex > smokeIndex;
		...
		const retriesRegistryPropagation =
			smokeStep.includes("for attempt in $(seq 1 12)") && smokeStep.includes("registry propagation");
		...
		const assertsDryRunRouting =
			smokeStep.includes('npx -y "$package_spec" --dry-run install --no-tui --codex-autonomous') &&
		...
		// #then
		expect(
			smokeRunsAfterPublishBeforeRestore,
			"post-publish smoke must run after lazycodex publish and before package restore",
		).toBe(true);
	...
	test("builds the Codex plugin components in publish-main before any package that ships the plugin tree", () => {
		...
		const buildStepSection =
			buildStepStart >= 0
				? publishMainJob.slice(buildStepStart, publishMainJob.indexOf("- name:", buildStepStart + 1))
				: "";
		...
		const buildsBeforeOpencodePublish = buildComponentsIndex >= 0 && opencodePublishIndex > buildComponentsIndex;
		const buildsBeforeOpenagentPublish = buildComponentsIndex >= 0 && openagentPublishIndex > buildComponentsIndex;
		const buildsBeforeLazycodexPublish = buildComponentsIndex >= 0 && lazycodexPublishIndex > buildComponentsIndex;
