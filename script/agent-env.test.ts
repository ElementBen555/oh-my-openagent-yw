...
function createCleanupHookFixture(workerBody: string): {
...
describe("agent dev-environment scripts", () => {
	...
	describe("cleanup.sh", () => {
		...
		posixBashTest(
			"#given the Claude SessionEnd launcher #when sync mode runs #then it executes cleanup and exits successfully",
			() => {
				// given
...
`);
		...
		posixBashTest(
			"#given the Claude SessionEnd launcher #when async mode runs #then it returns before cleanup finishes",
			async () => {
				// given
				const fixture = createCleanupHookFixture(`#!/usr/bin/env bash
