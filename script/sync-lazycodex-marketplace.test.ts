...
async function writePluginFixture(sourceRoot: string, options: WritePluginFixtureOptions = {}): Promise<void> {
	...
	await writeJson(
		join(sourceRoot, "packages", "omo-codex", "plugin", "components", "bootstrap", "hooks", "hooks.json"),
		{
			hooks: {
				SessionStart: [
					{
	...
	await writeFile(
		join(sourceRoot, "packages", "omo-codex", "plugin", "skills", astGrepSkillName, "SKILL.md"),
		"# AST-Grep\n",
	);
	...
	if (options.includeLazycodexRepositoryWorkflow ?? true) {
		await mkdir(join(sourceRoot, "packages", "omo-codex", "lazycodex-repository", ".github", "workflows"), {
			recursive: true,
		});
		await writeFile(
			join(
	...
	await writeFile(
		join(sourceRoot, "packages", "omo-codex", "plugin", "components", "lsp", "dist", "cli.js"),
		"#!/usr/bin/env node\n",
	);
	await mkdir(join(sourceRoot, "packages", "omo-codex", "plugin", "components", "comment-checker", "dist"), {
		recursive: true,
	});
	await writeFile(
		join(sourceRoot, "packages", "omo-codex", "plugin", "components", "comment-checker", "dist", "cli.js"),
		"#!/usr/bin/env node\n",
	);
	await mkdir(join(sourceRoot, "packages", "omo-codex", "plugin", "components", "bootstrap", "dist"), {
		recursive: true,
	});
	await writeFile(
		join(sourceRoot, "packages", "omo-codex", "plugin", "components", "bootstrap", "dist", "cli.js"),
		"#!/usr/bin/env node\n",
	);
	await mkdir(join(sourceRoot, "packages", "omo-codex", "plugin", "components", "bootstrap", "scripts"), {
		recursive: true,
	});
	await writeFile(
		join(sourceRoot, "packages", "omo-codex", "plugin", "components", "bootstrap", "scripts", "bootstrap.ps1"),
		"exit 0\n",
	);
	...
	await writeFile(
		join(sourceRoot, "packages", "omo-codex", "plugin", "node_modules", "ignored", "file.txt"),
		"ignored\n",
	);
...
describe("sync-lazycodex-marketplace", () => {
	test("#given marketplace sync #when copying plugin bundle #then emits self-contained mcp paths", async () => {
		...
		const marketplace = JSON.parse(
			await readFile(join(lazycodexRoot, ".agents", "plugins", "marketplace.json"), "utf8"),
		);
		expect(marketplace.name).toBe("sisyphuslabs");
		expect(marketplace.plugins[0].source).toBe("./plugins/omo");
		const manifest = JSON.parse(
			await readFile(join(lazycodexRoot, "plugins", "omo", ".codex-plugin", "plugin.json"), "utf8"),
		);
		...
		expect((await stat(join(lazycodexRoot, "plugins", "omo", "skills", astGrepSkillName, "SKILL.md"))).isFile()).toBe(
			true,
		);
		expect(
			(await stat(join(lazycodexRoot, "plugins", "omo", "components", "git-bash-mcp", "dist", "cli.js"))).isFile(),
		).toBe(true);
		expect(
			(await stat(join(lazycodexRoot, "plugins", "omo", "components", "lsp-tools-mcp", "dist", "cli.js"))).isFile(),
		).toBe(true);
		expect(
			(await stat(join(lazycodexRoot, "plugins", "omo", "components", "lsp-daemon", "dist", "cli.js"))).isFile(),
		).toBe(true);
	...
	test("#given release version env #when syncing marketplace #then repository payload is stamped with release version", async () => {
		...
		await writeJson(
			join(sourceRoot, "packages", "omo-codex", "plugin", "components", "comment-checker", "hooks", "hooks.json"),
			{
				hooks: {
					PostToolUse: [
						{
		...
		const componentHooks = JSON.parse(
			await readFile(
				join(lazycodexRoot, "plugins", "omo", "components", "comment-checker", "hooks", "hooks.json"),
				"utf8",
			),
		);
	...
	test("#given complete tree with bootstrap command and commandWindows targets #when syncing marketplace #then bundle validation passes", async () => {
		...
		// then
		expect(
			(await stat(join(lazycodexRoot, "plugins", "omo", "components", "bootstrap", "dist", "cli.js"))).isFile(),
		).toBe(true);
		expect(
			(
				await stat(join(lazycodexRoot, "plugins", "omo", "components", "bootstrap", "scripts", "bootstrap.ps1"))
			).isFile(),
		).toBe(true);
	...
	test("#given multiple missing referenced targets #when syncing marketplace #then reports the full list", async () => {
		...
		await writePluginFixture(sourceRoot);
		await rm(join(sourceRoot, "packages", "omo-codex", "plugin", "components", "bootstrap", "dist"), {
			recursive: true,
			force: true,
		});
		await rm(join(sourceRoot, "packages", "omo-codex", "plugin", "components", "bootstrap", "scripts"), {
			recursive: true,
			force: true,
		});
	...
	test("#given a missing lsp-daemon dist without the flag #then still hard-throws", async () => {
		...
		// when/then
		await expect(syncLazycodexMarketplace({ sourceRoot, lazycodexRoot })).rejects.toThrow(
			/missing built LSP daemon dist/,
		);
	});
});
