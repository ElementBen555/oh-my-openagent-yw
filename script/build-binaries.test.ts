...
async function writeFakeCli(tempDir: string): Promise<void> {
	...
	await writeFile(
		cliPath,
		["#!/usr/bin/env node", 'console.log(`bun-cli ${process.argv.slice(1).join(" ")}`);', ""].join("\n"),
	);
...
describe("build-binaries", () => {
	describe("PLATFORMS array", () => {
		it("includes baseline variants for non-AVX2 CPU support", async () => {
			...
			// when
			const hasAllBaselineTargets = EXPECTED_BASELINE_TARGETS.every((baseline) => targets.includes(baseline));
		...
		it("includes a windows-arm64 entry for Windows-on-ARM hosts", async () => {
			// given
			const module = await import("./build-binaries.ts");
			const platforms = (module as { PLATFORMS: { platform: string; packageName: string; packageDir: string }[] })
				.PLATFORMS;
		...
		it("uses JavaScript launcher names for baseline platforms", async () => {
			// given
			const module = await import("./build-binaries.ts");
			const platforms = (module as { PLATFORMS: { packageDir: string; target: string; binary: string }[] })
				.PLATFORMS;
		...
		it("launcher routes lazycodex install through the Node installer before requiring Bun", async () => {
			// given
			const module = await import("./build-binaries.ts");
			const createPlatformLauncherSource = (module as { createPlatformLauncherSource: () => string })
		...
		it("launcher routes omo codex-only install through the Node installer before requiring Bun", async () => {
			...
			await writeFile(
				installerPath,
				["#!/usr/bin/env node", 'console.log(`node-installer ${process.argv.slice(2).join(" ")}`);', ""].join("\n"),
			);
