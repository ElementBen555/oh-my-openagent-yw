...
const workflowChecks = [
	{
		path: ciWorkflowPath,
		testRuns: ["run: bun test", "run: bun test packages/omo-opencode/src/shared/dist-bundle-bun-globals.test.ts"],
	},
	{
...
describe("test workflows", () => {
	...
	test("prepares vendored lsp-tools-mcp before publish workflow tests and typecheck", () => {
		...
		const testBuildsLspToolsMcp =
			testJob.includes("name: Build vendored lsp-tools-mcp package") &&
		...
		const typecheckBuildsLspToolsMcp =
			typecheckJob.includes("name: Build vendored lsp-tools-mcp package") &&
		...
		expect(
			typecheckBuildsLspToolsMcp,
			"publish typecheck job must build lsp-tools-mcp before bun run typecheck",
		).toBe(true);
	});
	...
	test("runs Codex compatibility checks before publish jobs", () => {
		...
		expect(buildsLspToolsMcp, "publish Codex compatibility must build lsp-tools-mcp before bun run test:codex").toBe(
			true,
		);
	...
	test("runs codex compatibility checks on every supported os without serializing build", () => {
		...
		const writeGateNeedsAllChecks = autoCommitSchemaJob.includes(
			"needs: [test, typecheck, codex-compatibility, build]",
		);
		const draftReleaseNeedsAllChecks = draftReleaseJob.includes(
			"needs: [test, typecheck, codex-compatibility, build]",
		);
		...
		expect(
			buildWaitsForChecks,
			"Build has no artifact dependency on test/typecheck/codex and must not serialize CI",
		).toBe(false);
		expect(
	...
	test("sets up Bun before vendored lsp-tools-mcp builds", () => {
		...
		const ciCodexCompatibilityJob = sliceWorkflowSection(
			ciWorkflow,
			"  codex-compatibility:",
			"  lazycodex-published-smoke:",
		);
		...
		const publishCodexCompatibilityJob = sliceWorkflowSection(
			publishWorkflow,
			"  codex-compatibility:",
			"  preflight-trust:",
		);
	...
	test("tracks the nested Codex plugin lockfile used by npm ci", () => {
		...
		// #then
		expect(lockfileIsUnignored, "the aggregate Codex plugin lockfile must escape the root package-lock ignore").toBe(
			true,
		);
		expect(trackedLockfile, "npm ci in CI requires the nested Codex plugin package-lock.json to be tracked").toBe(
	...
	test("publishes platform packages before installable wrappers", () => {
		...
		const platformUsesMetadata =
			workflow.includes("version: ${{ needs.release-metadata.outputs.version }}") &&
			workflow.includes("dist_tag: ${{ needs.release-metadata.outputs.dist_tag }}");
		const mainWaitsForPlatform =
			workflow.includes(
				"needs: [test, typecheck, codex-compatibility, preflight-trust, release-metadata, prepare-release-state, publish-platform]",
	...
	test("runs published lazycodex-ai smoke commands from a clean external directory", () => {
		...
		const smokeIsNonBlocking =
			workflow.includes("lazycodex-published-smoke:") && workflow.includes("continue-on-error: true");
		const hasExternalSmokeDir =
			workflow.includes("SMOKE_DIR=$(mktemp -d)") && workflow.includes('cd "$SMOKE_DIR/cwd"');
		...
		const runsNpxDoctorSmoke = workflow.includes("npx -y lazycodex-ai@latest --dry-run doctor");
		...
		expect(removedStrictInstallGate, "publish workflow must not use the strict lazycodex install equality gate").toBe(
			true,
		);
		expect(removedStrictDoctorGate, "publish workflow must not use the strict lazycodex doctor equality gate").toBe(
			true,
		);
	});
	...
	test("fails when a required platform artifact is missing", () => {
		...
		expect(suppressesDownloadFailure, "missing required artifacts must fail the reusable publish workflow").toBe(
			false,
		);
	});
	...
	test("keeps the platform publish workflow step syntax valid around version updates", () => {
		...
		// #then
		expect(duplicateVersionStep, "platform publish workflow must not contain adjacent duplicate step names").toBe(
			false,
		);
	});
	...
	test("regenerates and commits bun.lock in the release version bump", () => {
		...
		// #when
		const applyStep = sliceWorkflowSection(
			workflow,
			"      - name: Apply release version to source tree",
			"      - name: Commit version bump",
		);
		const commitStep = sliceWorkflowSection(
			workflow,
			"      - name: Commit version bump",
		...
		expect(applyStep.indexOf("bun install --lockfile-only")).toBeGreaterThan(
			applyStep.indexOf("node packages/omo-codex/plugin/scripts/sync-version.mjs"),
		);
	...
	test("keeps the release tail safe to rerun after a tag exists", () => {
		...
		// #when
		const hasExistingTagResolver =
			workflow.includes("id: release-state") &&
	...
	test("enumerates windows-arm64 consistently across every platform-list surface", () => {
		...
		expect(
			...
			"publish.yml must enumerate platforms in 2 PLATFORMS arrays + 3 version-bump loops",
		).toBe(5);
	...
	test("matches the canonical platform set in optionalDependencies and on-disk platform packages", () => {
		...
		expect(onDiskPlatforms, "packages/ must contain a directory for every canonical platform package").toEqual(
			buildBinariesPlatforms,
		);
	});
});
