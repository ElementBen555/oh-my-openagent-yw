...
describe("session-registry", () => {
	...
	test("keeps the registry path stable after first path resolution", () => {
		...
		try {
			...
			expect(
				existsSync(join(otherDataHome, "opencode", "storage", "openclaw", "reply-session-registry.jsonl")),
			).toBe(false);
		} finally {
	...
	test("#given XDG_DATA_HOME #when resolving registry paths #then the path bytes match OpenCode storage plus openclaw", () => {
		...
		expect(getRegistryPath()).toBe(
			join(tempDataHome, "opencode", "storage", "openclaw", "reply-session-registry.jsonl"),
		);
