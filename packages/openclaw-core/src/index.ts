...
import { interpolateInstruction, wakeCommandGateway, wakeGateway } from "./dispatcher";
	...
	try {
		...
		if (!tmuxTail && (event === "stop" || event === "session-end") && process.env.TMUX) {
			try {
				...
				if (DEBUG) {
					console.error("[openclaw] failed to capture tmux tail:", error instanceof Error ? error.message : error);
				}
			}
		}
