# src/mcp/ — 7 Built-in MCPs

**Generated:** 2026-05-18

## OVERVIEW

Tier 1 of the three-tier MCP system. Built-ins are created by `createBuiltinMcps(disabledMcps, config, options)` and include remote MCPs plus the local `lsp`, `codegraph`, `sequential_thinking`, and `github` stdio MCPs.
...
| **sequential_thinking** | local (stdio) | `npx -y @modelcontextprotocol/server-sequential-thinking` | None | Dynamic reflective problem-solving through thought sequences |
| **github** | local (stdio) | `npx -y @github/github-mcp-server` | `GITHUB_PERSONAL_ACCESS_TOKEN` (required) | Repository, issue, PR, branch, commit, code search, security scanning |

## OFFLINE RESILIENCE

Remote MCPs (`websearch`, `context7`, `grep_app`) are probed at plugin initialization with a 2-second HEAD request. Unreachable MCPs are automatically disabled (`enabled: false`) so the plugin continues to function offline. See `mcp-health-check.ts`.
...
|------|--------|-----------|
| 1. Built-in | `src/mcp/` | 3 remote HTTP MCPs + 4 local stdio MCPs (`lsp`, `codegraph`, `sequential_thinking`, `github`) via `createBuiltinMcps()` |
...
| `types.ts` | `McpNameSchema`: `"websearch" \| "context7" \| "grep_app" \| "lsp" \| "codegraph" \| "sequential_thinking" \| "github"` |
...
| `sequential-thinking.ts` | Sequential Thinking MCP config (stdio via npx) |
| `github.ts` | GitHub MCP config (stdio via npx; gated on `GITHUB_PERSONAL_ACCESS_TOKEN`) |
| `mcp-health-check.ts` | Offline resilience: probes remote MCPs at init, disables unreachable ones gracefully |
