...
import { type CodegraphMcpConfigOptions, createCodegraphMcpConfig } from "./codegraph";
...
export function createBuiltinMcps(
	...
	if (!disabledMcps.includes("sequential_thinking")) {
		mcps.sequential_thinking = createSequentialThinkingMcpConfig();
	}

	if (!disabledMcps.includes("github")) {
		const githubConfig = createGithubMcpConfig();
		if (githubConfig) {
			mcps.github = githubConfig;
		}
	}
