...
function getCliDefaultConfigDir(): string {
	...
	const shouldIgnoreWindowsXdg =
		envXdgConfig !== undefined &&
		envXdgConfig.length > 0 &&
		isWslEnvironment() &&
		isWindowsUserConfigRoot(envXdgConfig);
...
export function getOpenCodeConfigDirs(options: OpenCodeConfigDirOptions): string[] {
	...
	return Array.from(new Set([...(customConfigDir ? [customConfigDir] : []), getCliDefaultConfigDir()]));
}
