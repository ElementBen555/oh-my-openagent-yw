...
function findHardcodedFallbackEntry(providerID: string, modelID: string): FallbackEntry | undefined {
...
export function isHardcodedMultimodalFallbackModel(model: VisionCapableModel): boolean {
	return MULTIMODAL_LOOKER_REQUIREMENT.fallbackChain.some((entry) =>
		entry.providers.some(
			(providerID) => getFullModelKey(providerID, entry.model) === getFullModelKey(model.providerID, model.modelID),
		),
	);
}

export function buildMultimodalLookerFallbackChain(visionCapableModels: VisionCapableModel[]): FallbackEntry[] {
