...
describe("resolveMultimodalLookerAgentMetadata", () => {
	...
	test("returns configured multimodal-looker model when it already matches a vision-capable override", async () => {
		...
		setVisionCapableModelsCache(
			new Map([["rundao/public/qwen3.5-397b", { providerID: "rundao", modelID: "public/qwen3.5-397b" }]]),
		);
		spyOn(modelAvailability, "fetchAvailableModels").mockResolvedValue(new Set(["rundao/public/qwen3.5-397b"]));
	...
	test("returns registered model variant directly without merging from dynamic resolution", async () => {
		...
		setVisionCapableModelsCache(new Map([["openai/gpt-5.4", { providerID: "openai", modelID: "gpt-5.4" }]]));
		spyOn(modelAvailability, "fetchAvailableModels").mockResolvedValue(new Set(["openai/gpt-5.4"]));
	...
	test("falls back to the hardcoded multimodal chain when no dynamic vision model exists", async () => {
		...
		spyOn(modelAvailability, "fetchAvailableModels").mockResolvedValue(new Set(["google/gemini-3-flash"]));
