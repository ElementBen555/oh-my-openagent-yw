	...
	return tool({
		...
		args: {
			...
			file_paths: tool.schema
				...
				.optional()
				.describe("Absolute paths to the files to analyze"),
			image_data: tool.schema
				.string()
				.optional()
				.describe("Base64 encoded image data (for clipboard/pasted images)"),
			image_data_list: tool.schema
				.array(tool.schema.string())
				.optional()
				.describe("Base64 encoded image data entries (for multiple clipboard/pasted images)"),
