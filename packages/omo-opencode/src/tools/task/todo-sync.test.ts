...
import { syncAllTasksToTodos, syncTaskTodoUpdate, syncTaskToTodo, type TodoInfo } from "./todo-sync";

type MockTodoCtx = PluginInput & {
	client: {
		session: { todo: { mockResolvedValue: (value: unknown) => void; mockRejectedValue: (value: unknown) => void } };
	};
};
...
describe("syncTaskTodoUpdate", () => {
	...
	it("writes updated todo and preserves existing items", async () => {
		...
		const writer = async (input: { sessionID: string; todos: TodoInfo[] }) => {
			...
			expect(input.todos.find((todo: TodoInfo) => todo.id === "T-1")?.content).toBe("Updated task");
	...
	it("removes deleted task from todos", async () => {
		...
		const writer = async (input: { sessionID: string; todos: TodoInfo[] }) => {
			...
			expect(input.todos.some((todo: TodoInfo) => todo.id === "T-1")).toBe(false);
			expect(input.todos.some((todo: TodoInfo) => todo.id === "T-2")).toBe(true);
		};
