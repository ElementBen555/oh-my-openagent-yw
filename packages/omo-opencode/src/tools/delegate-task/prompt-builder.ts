...
function buildAvailableSkillsSection(skills: AvailableSkill[]): string {
	...
	const rows = skills.map((s) => `- \`${s.name}\`: ${s.description || s.name}`).join("\n");
...
export function buildSystemContent(input: BuildSystemContentInput): string | undefined {
	...
	const planAgentPrepend = isPlan ? buildPlanAgentSystemPrepend(availableCategories, effectiveAvailableSkills) : "";

	const skillsSection = !isPlan ? buildAvailableSkillsSection(effectiveAvailableSkills) : "";

	const baseAgentsContext = agentsContext ?? planAgentPrepend;
	const effectiveAgentsContext =
		!isPlan && skillsSection ? [baseAgentsContext, skillsSection].filter(Boolean).join("\n\n") : baseAgentsContext;

	const effectiveMaxPromptTokens =
		maxPromptTokens ?? (usesFreeOrLocalModel(model) ? FREE_OR_LOCAL_PROMPT_TOKEN_LIMIT : undefined);
