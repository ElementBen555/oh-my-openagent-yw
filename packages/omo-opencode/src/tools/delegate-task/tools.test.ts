...
describe("sisyphus-task", () => {
	...
	describe("background metadata sessionId", () => {
		test("should wait for background sessionId and set metadata for TUI toolcall counting", async () => {
			...
			const tasks = new Map<
				string,
				{ id: string; sessionID?: string; status: string; description: string; agent: string }
			>();
			const mockManager = {
				...
				const mockClient = {
					...
					session: {
						...
						messages: async () => ({
							data: [
								{
									info: { id: "msg_1", role: "assistant", time: { created: Date.now() }, finish: "end_turn" },
					...
					session: {
						...
						messages: async () => ({
							data: [
								{
									info: { role: "assistant", time: { created: Date.now() } },
									parts: [{ type: "text", text: "Minimax task completed successfully" }],
								},
							],
						}),
		...
		test(
			...
			async () => {
				...
				const mockClient = {
					app: {
						agents: async () => ({
							data: [{ name: "oracle", mode: "subagent", model: { providerID: "openai", modelID: "gpt-5.5" } }],
						}),
					},
					config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
					session: {
