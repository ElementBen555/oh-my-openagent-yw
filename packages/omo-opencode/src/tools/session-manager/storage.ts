...
function mergeSessionMetadataLists(sdkSessions: SessionMetadata[], fileSessions: SessionMetadata[]): SessionMetadata[] {
...
export async function getMainSessions(options: GetMainSessionsOptions): Promise<SessionMetadata[]> {
	...
	if (isSqliteBackend() && sdkClient) {
		try {
			...
			log("[session-manager] falling back to file session list after SDK unavailable error", {
				error: String(error),
			});
		}
	}
...
export async function getAllSessions(): Promise<string[]> {
	if (isSqliteBackend() && sdkClient) {
		try {
			...
			if (!shouldFallbackFromSdkError(error)) throw error;
			log("[session-manager] falling back to file session ids after SDK unavailable error", {
				error: String(error),
			});
		}
	}
...
export async function sessionExists(sessionID: string): Promise<boolean> {
	if (isSqliteBackend() && sdkClient) {
		try {
			...
			if (!shouldFallbackFromSdkError(error)) throw error;
			log("[session-manager] falling back to file sessionExists after SDK unavailable error", {
				error: String(error),
				sessionID,
			});
		}
...
export async function readSessionMessages(sessionID: string): Promise<SessionMessage[]> {
	if (isSqliteBackend() && sdkClient) {
		try {
			...
			if (!shouldFallbackFromSdkError(error)) throw error;
			log("[session-manager] falling back to file session messages after SDK unavailable error", {
				error: String(error),
				sessionID,
			});
		}
...
export async function readSessionTodos(sessionID: string): Promise<TodoItem[]> {
	if (isSqliteBackend() && sdkClient) {
		try {
			...
			if (!shouldFallbackFromSdkError(error)) throw error;
			log("[session-manager] falling back to file session todos after SDK unavailable error", {
				error: String(error),
				sessionID,
			});
		}
...
export async function getSessionInfo(sessionID: string): Promise<SessionInfo | null> {
	if (isSqliteBackend() && sdkClient) {
		try {
			...
			if (!shouldFallbackFromSdkError(error)) throw error;
			log("[session-manager] falling back to file session info after SDK unavailable error", {
				error: String(error),
				sessionID,
			});
		}
