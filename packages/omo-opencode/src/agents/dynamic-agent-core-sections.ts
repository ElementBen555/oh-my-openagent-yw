import type { AvailableAgent, AvailableCategory, AvailableSkill, AvailableTool } from "./dynamic-agent-prompt-types";
 ...
 */
export function buildAgentIdentitySection(agentName: string, roleDescription: string): string {
...
export function buildToolSelectionTable(
	...
	const sortedAgents = [...agents]
		...
		.sort((left, right) => costOrder[left.metadata.cost] - costOrder[right.metadata.cost]);
...
export function buildFrontendGuidanceSection(categories: AvailableCategory[]): string {
	const hasVisualEngineeringCategory = categories.some((category) => category.name === "visual-engineering");
