...
function permissionValue(permission: ReturnType<typeof createSisyphusAgent>["permission"], key: string): unknown {
...
describe("createSisyphusAgent", () => {
	...
	describe("#given a Gemini model", () => {
		test("#when creating the agent #then injects Gemini corrective anchors before constraints", () => {
			...
			expect(prompt.indexOf("<GEMINI_DELEGATION_OVERRIDE>")).toBeLessThan(prompt.indexOf("<Constraints>"));
