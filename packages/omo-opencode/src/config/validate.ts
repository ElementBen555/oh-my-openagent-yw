...
function schemaMessages(configPath: string, rawConfig: Record<string, unknown>): readonly string[] {
	...
	return result.error.issues.map(
		(issue) => `${shortPath(configPath)}: ${formatIssuePath(issue.path)}: ${issue.message}`,
	);
}
...
export function validatePluginConfig(directory: string): PluginConfigValidation {
	...
	const projectLayersNearestFirst = discoverProjectLayersNearestFirst(directory).map((layer) =>
		parseLayerConfig(layer.path),
	);
