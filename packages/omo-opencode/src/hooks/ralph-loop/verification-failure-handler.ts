...
async function getSessionMessageCount(ctx: PluginInput, sessionID: string, directory: string): Promise<number> {
...
export async function handleFailedVerification(
	...
	const clearedState = loopState.clearVerificationState(parentSessionID, messageCountAtStart);
	if (!clearedState) {
