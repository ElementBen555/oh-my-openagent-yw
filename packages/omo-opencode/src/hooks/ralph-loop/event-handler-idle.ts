...
import { hasActiveBackgroundTasks, isSyntheticIdle, RAPID_IDLE_DEDUP_MS } from "./event-handler-activity";
import { handleCompletionIfDetected } from "./event-handler-completion";
import { continueSettledIteration, stopIfLatestAssistantMadeNoProgress } from "./event-handler-continuation";
...
export function getVerificationSessionID(state: RalphLoopState): string | undefined {
	return state.verification_pending ? state.verification_session_id : undefined;
}
...
export function maxIterationsReached(state: RalphLoopState): boolean {
	return typeof state.max_iterations === "number" && state.iteration >= state.max_iterations;
}
...
function shouldSkipRapidRealIdle(
	...
	if (
		!syntheticIdle &&
		lastHandledSyntheticIdleAt !== undefined &&
		now - lastHandledSyntheticIdleAt < RAPID_IDLE_DEDUP_MS
	) {
