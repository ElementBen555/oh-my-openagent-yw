...
async function readMessagesFromSDK(client: OpencodeClient, sessionID: string): Promise<MessagePart[]> {
	try {
		...
		const rawMessages = normalizeSDKResponse(response, [] as Array<{ parts?: ToolPart[] }>, {
...
export async function executeDeduplication(
	...
	const messages =
		client && isSqliteBackend() ? await readMessagesFromSDK(client, sessionID) : readMessages(sessionID);
