...
import { endTaskTimer, getWorkForSession, resolveBoulderPlanPathForWork } from "../../features/boulder-state";
...
export async function handleDirectWorkToolAfter(input: {
	...
	const planSnapshot =
		toolInput.callID && pendingPlanSnapshots ? pendingPlanSnapshots.get(toolInput.callID) : undefined;
	if (toolInput.callID) {
