import { findMatchingHooks, log, objectToSnakeCase, transformToolName } from "../../shared";
		...
		for (const hook of matcher.hooks) {
			...
			if (result.exitCode === 2) {
				return {
					decision: "deny",
					reason:
						normalizeHookText(result.stderr) ?? normalizeHookText(result.stdout) ?? "Hook blocked the operation",
			...
			if (result.stdout) {
				try {
					...
					if (output.stopReason !== undefined)
						accumulatedCommonFields.stopReason = normalizeHookText(output.stopReason);
					if (output.suppressOutput !== undefined) accumulatedCommonFields.suppressOutput = output.suppressOutput;
					if (output.systemMessage !== undefined)
						accumulatedCommonFields.systemMessage = normalizeHookText(output.systemMessage);
				} catch (error) {
