...
import { executeUserPromptSubmitHooks, type MessagePart, type UserPromptSubmitContext } from "../user-prompt-submit";

export function createChatMessageHandler(ctx: PluginInput, config: PluginConfig, contextCollector?: ContextCollector) {
	return async (
		input: {
			sessionID: string;
			agent?: string;
