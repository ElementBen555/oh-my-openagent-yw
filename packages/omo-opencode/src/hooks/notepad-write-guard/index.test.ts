...
async function invoke(hook: Hook, args: { tool: string; filePath: string }): Promise<void> {
	await hook["tool.execute.before"]?.({ tool: args.tool } as never, { args: { filePath: args.filePath } } as never);
}
