...
import { createFirstPromptWatchdog, type FirstPromptWatchdog, observeEventForWatchdog } from "./first-prompt-watchdog";
...
describe("observeEventForWatchdog", () => {
	...
	it.each(
		assistantProgressParts,
	)("#given a message.updated assistant event whose only part is type=%s #when observed #then onAssistantProgress is called (model is *working*, not silent)", (_label: string, part: {
		readonly type: string;
		readonly text?: string;
		readonly id?: string;
		readonly name?: string;
	...
	it.each(
		assistantProgressParts,
	)("#given a message.part.updated event whose part is type=%s #when observed #then onAssistantProgress is called", (_label: string, part: {
		readonly type: string;
		readonly text?: string;
		readonly id?: string;
		readonly name?: string;
	...
	it.each(
		terminalEventTypes,
	)("#given a %s event #when observed #then onSessionTerminal is called", (eventType: string) => {
		const calls = freshCalls();
		observeEventForWatchdog({ type: eventType, properties: { sessionID } }, createRecordingWatchdog(calls));
