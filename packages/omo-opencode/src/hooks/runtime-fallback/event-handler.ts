...
function resolvePreferredSessionModel(
	...
	const category =
		typeof agentConfig?.category === "string" ? agentConfig.category : SessionCategoryRegistry.get(sessionID);
	...
	const handleSessionError = async (props: Record<string, unknown> | undefined) => {
		...
		if (isAbortError(error)) {
			...
			if (deps.internallyAbortedSessions.has(sessionID)) {
				...
				log(`[${HOOK_NAME}] session.error matched internal abort; preserving retry state`, {
					sessionID,
					resolvedAgent,
				});
				return;
