...
export function createAutoRetryDispatcher(
	...
	const { ctx, sessionStates, sessionRetryInFlight, sessionAwaitingFallbackResult, pluginConfig } = deps;
	...
	return async (
		...
		const previousPendingFallbackPromptMayHaveBeenAccepted =
			sessionStates.get(sessionID)?.pendingFallbackPromptMayHaveBeenAccepted;
