...
function getSessionCache(sessionCaches: Map<string, Set<string>>, sessionID: string): Set<string> {
...
export async function processFilePathForReadmeInjection(input: {
	...
	for (const readmePath of readmePaths) {
		...
		try {
			...
			const errorMessage = error instanceof Error ? error.message : describeReadmeInjectionError(error);
