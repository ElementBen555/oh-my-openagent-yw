...
export function findPlanByName(plans: readonly string[], requestedName: string): string | null {
	...
	return (
		plans.find((planPath) => normalizePlanLookupValue(getPlanName(planPath)).includes(normalizedRequestedName)) ??
		null
	);
}
...
export function formatIncompletePlanList(plans: readonly string[], includeModifiedTime: boolean): string {
	return plans
		.map((planPath, index) => {
