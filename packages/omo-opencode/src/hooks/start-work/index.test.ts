...
describe("start-work hook", () => {
	...
	function createStartWorkPrompt(options?: { sessionContext?: string; userRequest?: string }): string {
...
<user-request>${userRequest}</user-request>`
	...
	describe("session agent management", () => {
		...
		test("should fall back to Sisyphus instead of keeping Prometheus when Atlas is unavailable", async () => {
			...
			// when
			await hook["chat.message"]({ sessionID: "ses-prometheus-to-worker" }, output);
	...
	describe("retry-path idempotence (#4480)", () => {
		test("should substitute $SESSION_ID / $TIMESTAMP across all text parts", async () => {
				...
				parts: [
					{
						...
						text: createStartWorkPrompt({
							sessionContext: "Session: ses-abc123\nTimestamp: 2026-01-01T00:00:00Z",
						}),
					},
					{
						type: "text",
			...
			// when
			await hook["chat.message"]({ sessionID: "ses-retry-789" }, output);
