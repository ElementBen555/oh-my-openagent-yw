...
function createFakeTimers(): FakeTimers {
	...
	const timers = new Map<
		number,
		{ id: number; time: number; interval: number | null; callback: TimerCallback; args: unknown[] }
	>();
...
describe("todo-continuation-enforcer", () => {
	...
	test(
		"should clear max failure state on session deleted",
		async () => {
			//#given
	...
	test(
		...
		async () => {
			...
			const mockInput = createMockPluginInput();
			mockInput.client.session.todo = async () => ({
				data: [{ id: "1", content: "Write tests", status: "pending", priority: "high" }],
			});
