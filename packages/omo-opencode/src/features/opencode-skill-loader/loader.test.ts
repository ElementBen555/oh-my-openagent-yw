...
`;
			...
			try {
				...
				expect(skill?.mcpConfig?.sqlite?.args).toEqual(["mcp-server-sqlite", "--db-path", "./data.db"]);
...
Skill body.
`;
			const singularSkillDir = join(TEST_DIR, ".opencode", "skill", "singular-opencode-skill");
