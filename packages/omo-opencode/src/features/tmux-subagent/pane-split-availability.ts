...
export function isSplittableAtCount(agentAreaWidth: number, paneCount: number, minPaneWidth?: number): boolean {
...
export function canSplitPane(pane: TmuxPaneInfo, direction: SplitDirection, minPaneWidth?: number): boolean {
	if (direction === "-h") {
...
export function canSplitPaneAnyDirection(pane: TmuxPaneInfo, minPaneWidth?: number): boolean {
	return pane.width >= getMinSplitWidth(minPaneWidth) || pane.height >= MIN_SPLIT_HEIGHT;
}

export function getBestSplitDirection(pane: TmuxPaneInfo, minPaneWidth?: number): SplitDirection | null {
