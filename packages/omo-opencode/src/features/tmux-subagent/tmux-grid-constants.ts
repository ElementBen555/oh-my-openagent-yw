...
export function computeMainPaneWidth(windowWidth: number, config?: CapacityConfig): number {
...
export function computeAgentAreaWidth(windowWidth: number, config?: CapacityConfig): number {
	const safeWindowWidth = Math.max(0, windowWidth);
	if (!config) {
