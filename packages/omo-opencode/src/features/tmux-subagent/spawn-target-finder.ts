...
function findSplittableTarget(
	...
	if (!isStrictMainVertical(config) && leftPane && canSplitPane(leftPane, "-h", minAgentPaneWidth)) {
...
export function findSpawnTarget(state: WindowState, config: CapacityConfig): SpawnTarget | null {
