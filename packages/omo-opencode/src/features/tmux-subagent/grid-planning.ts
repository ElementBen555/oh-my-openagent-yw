import { computeAgentAreaWidth, DIVIDER_SIZE, MAX_GRID_SIZE } from "./tmux-grid-constants";
...
export function calculateCapacity(
	...
	const cols = Math.min(
		MAX_GRID_SIZE,
		Math.max(0, Math.floor((availableWidth + DIVIDER_SIZE) / (minPaneWidth + DIVIDER_SIZE))),
	);
	const rows = Math.min(
		MAX_GRID_SIZE,
