...
export async function tryFallbackRetry(args: {
	...
	while (fallbackChain && selectedAttemptCount < fallbackChain.length) {
		...
		const candidateProviderID = deps.selectFallbackProvider(candidate.providers, task.model?.providerID);
	...
	const providerID = nextProviderID ?? deps.selectFallbackProvider(nextFallback.providers, task.model?.providerID);
