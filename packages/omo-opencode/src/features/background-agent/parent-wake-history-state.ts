...
export function latestAssistantTurnHasToolBlock(messages: readonly unknown[]): boolean {
	for (let index = messages.length - 1; index >= 0; index--) {
		...
		if (role === "assistant") {
			return (
				messageFinish(message) === "tool-calls" ||
...
function partHasFreshToolActivity(part: unknown, now: number, maxAgeMs: number): boolean {
	...
	return (
		timeHasFreshActivity(part.time, now, maxAgeMs) ||
		timeHasFreshActivity(isRecord(part.state) ? part.state.time : undefined, now, maxAgeMs)
	);
}
