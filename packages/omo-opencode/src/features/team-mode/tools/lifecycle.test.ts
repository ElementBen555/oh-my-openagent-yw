...
describe("team lifecycle tools", () => {
	...
	test("team_create works without toolContext.client field", async () => {
		...
		// when
		const result = parseToolResult<{ teamRunId: string; runtimeState: RuntimeState }>(
	...
	test("team_create normalizes inline lead shorthand before creating the runtime", async () => {
		...
		const result = parseToolResult<{ runtimeState: RuntimeState }>(
			await teamCreateTool.execute({ inline_spec: inlineSpec }, createToolContext("lead-session")),
		);
	...
	test("team_create checks participant conflicts against the effective leadSessionId override", async () => {
		...
		await teamCreateTool.execute(
			{ inline_spec: createSpec(), leadSessionId: "effective-lead-session" },
		...
		const result = teamCreateTool.execute(
			{ inline_spec: betaSpec, leadSessionId: "effective-lead-session" },
	...
	test("team_delete propagates active-member errors", async () => {
		...
		const created = parseToolResult<{ teamRunId: string }>(
			await createTool.execute({ inline_spec: createSpec() }, createToolContext("lead-session")),
		);
	...
	test("team_delete force=true succeeds even with active members", async () => {
		...
		const result = parseToolResult<{ deleted: boolean }>(
			await deleteTool.execute({ teamRunId: created.teamRunId, force: true }, createToolContext("lead-session")),
		);
	...
	test("runs full lifecycle through create, request, approve, and delete", async () => {
		...
		const deleteTool = createTeamDeleteTool(config, mockClient, backgroundManager, undefined, lifecycleDeps);
		const created = parseToolResult<{ teamRunId: string; runtimeState: RuntimeState }>(
			await createTool.execute({ inline_spec: createSpec() }, createToolContext("lead-session")),
		);
		...
		const requestResult = parseToolResult<{ status: string }>(
			await requestTool.execute(
				{ teamRunId: created.teamRunId, targetMemberName: "member-a" },
				createToolContext("lead-session"),
			),
		);
		const approveResult = parseToolResult<{ status: string }>(
			await approveTool.execute(
				{ teamRunId: created.teamRunId, memberName: "member-a" },
				createToolContext(memberSessionId ?? "member-a-session"),
			),
		);
		const deleteResult = parseToolResult<{ deleted: boolean }>(
			await deleteTool.execute({ teamRunId: created.teamRunId }, createToolContext("lead-session")),
		);
	...
	test("team_reject_shutdown records the rejection reason", async () => {
		...
		const created = parseToolResult<{ teamRunId: string; runtimeState: RuntimeState }>(
			await createTool.execute({ teamName: "alpha-team" }, createToolContext("lead-session")),
		);
		const memberSessionId = created.runtimeState.members.find((member) => member.name === "member-a")?.sessionId;
		await requestTool.execute(
			{ teamRunId: created.teamRunId, targetMemberName: "member-a" },
		...
		const result = parseToolResult<{
			...
			await rejectTool.execute(
				{ teamRunId: created.teamRunId, memberName: "member-a", reason: "still working" },
				createToolContext(memberSessionId ?? "member-a-session"),
			),
		);
		...
		expect(getLatestShutdownRequest(requireRuntime(created.teamRunId), "member-a")).toEqual(
			expect.objectContaining({ rejectedReason: "still working", rejectedAt: expect.any(Number) }),
		);
	});
