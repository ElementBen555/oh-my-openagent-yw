import type { listActiveTeams, loadRuntimeState } from "@oh-my-opencode/team-core/team-state-store/store";
...
export function sanitizeRuntimeState(runtimeState: RuntimeState): Omit<RuntimeState, "members"> & {
	...
	return {
		...
		members: runtimeState.members.map(
			({ lastInjectedTurnMarker: _turnMarker, pendingInjectedMessageIds: _pendingIds, ...member }) => member,
		),
	};
}
...
export async function findParticipantRuntime(
	...
	if (registryEntry) {
		const runtimeState = await deps
			.loadRuntimeState(registryEntry.teamRunId, config)
			.catch(resolveRuntimeStateLoadFailure);
		if (
			runtimeState &&
			ACTIVE_RUNTIME_STATUSES.has(runtimeState.status) &&
	...
	for (const activeTeam of await deps.listActiveTeams(config)) {
		const runtimeState = await deps
			.loadRuntimeState(activeTeam.teamRunId, config)
			.catch(resolveRuntimeStateLoadFailure);
