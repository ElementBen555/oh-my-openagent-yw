...
const TeamCreateInlineMemberToolSchema = tool.schema.object({
	name: tool.schema
		.string()
		.optional()
		.describe("Member name, kebab-case or natural text; normalized before team creation."),
	kind: tool.schema
		.enum(["category", "subagent_type"])
		.optional()
		.describe(
	...
	subagent_type: tool.schema
		.string()
		.optional()
		.describe("Required for subagent_type members. Eligible examples: sisyphus, atlas, sisyphus-junior."),
	prompt: tool.schema
		.string()
		.optional()
		.describe("Task prompt for this member. Category members need a concrete work prompt."),
	...
	role: tool.schema
		.string()
		.optional()
		.describe("Optional natural-language role used to build a prompt when prompt is omitted."),
	description: tool.schema
		.string()
		.optional()
		.describe("Optional natural-language description used to build a prompt when prompt is omitted."),
});
...
const TeamCreateInlineSpecToolSchema = tool.schema.union([
	tool.schema.object({
		...
		members: tool.schema
			.array(TeamCreateInlineMemberToolSchema)
			.describe("Team members; members must be a flat array, not an object or nested groups. Provide 1-8 members."),
		teamAllowedPaths: tool.schema
	...
	return tool({
		...
		args: {
			teamName: tool.schema
				.string()
				.optional()
				.describe("Named team spec to load. Provide exactly one of teamName or inline_spec."),
		...
		async execute(rawArgs, toolContext) {
			...
			if (callerTeamLead.displayName !== undefined) {
				...
				if (callerRegistryEntry?.verdict === "hard-reject") {
					throw new Error(
						`team_create denied: caller '${callerAgentKey}' is a hard-reject agent and cannot create teams regardless of an explicit 'lead' in the spec. ${callerRegistryEntry.rejectionMessage ?? `Agent '${callerAgentKey}' is not eligible to lead a team.`}`,
					);
				}
			}
			...
			if (
				participantRuntime &&
				(participantRuntime.teamName !== spec.name || participantRuntime.leadSessionId !== leadSessionId)
			) {
				throw new Error(
