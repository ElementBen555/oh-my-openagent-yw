...
describe("team-mode integration", () => {
	test("C-10.1 creates a single-member echo team, delivers mail, surfaces unread status, and deletes runtime", async () => {
		...
		const runtime = await createTeamRun(
			createSpec("echo-team", "echo", [
				{ kind: "subagent_type", name: "echo", subagent_type: "atlas", backendType: "in-process", isActive: true },
			]),
			"ses_lead",
	...
	test("C-10.2 runs a 2-member pipeline where worker claims and completes a lead-created task", async () => {
		...
		const createdTask = await createTask(
			runtime.teamRunId,
			{ subject: "X", description: "Ship X", blocks: [], blockedBy: [], status: "pending" },
			config,
		);
	...
	test("C-10.3 resumes alive teams, orphans dead leads, fails stuck creating teams, and cleans deleting runs", async () => {
		...
		await saveRuntimeState(
			{ ...(await loadRuntimeState(deletingRuntime.teamRunId, config)), status: "deleting" },
			config,
		);
	...
	test("C-10.4 keeps member spawn concurrency within max_parallel_members", async () => {
		...
		await createTeamRun(
			...
			"ses_lead",
			createContext(baseDir, manager, new Set(["ses_lead"])),
			createConfig(baseDir, { max_parallel_members: 2 }),
			manager,
		);
