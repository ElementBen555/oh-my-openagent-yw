...
import { type DeleteTeamBackgroundManager, deleteTeam } from "./delete-team";
...
describe("deleteTeam cancels only this team's background tasks", () => {
	...
	test("uses leadSessionId as the getTasksByParentSession key", async () => {
		...
		const cancelTaskMock = mock(
			async (_taskId: string, _options?: Parameters<DeleteTeamBackgroundManager["cancelTask"]>[1]) => true,
		);
		const bgMgr = {
