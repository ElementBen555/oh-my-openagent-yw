...
async function findExistingRuntime(
	...
	for (const candidate of await listActiveTeams(config)) {
		if (candidate.teamName !== spec.name || (candidate.status !== "creating" && candidate.status !== "active"))
			continue;
...
async function updateMemberInRuntimeState(
	...
	return transitionRuntimeState(
		...
		(currentState) => ({
			...currentState,
			members: currentState.members.map((member) => (member.name === memberName ? patch(member) : member)),
		}),
		config,
	);
}
				...
				while (!failure) {
					...
					try {
						...
						const task = await bgMgr.launch({
							...
							parentMessageId:
								options?.parentMessageID ?? `team-create:${runtimeState.teamRunId}:${member.name}`,
							teamRunId: runtimeState.teamRunId,
