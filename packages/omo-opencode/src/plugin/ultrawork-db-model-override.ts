...
function tryUpdateMessageModel(
	...
	try {
		...
		finalizeStatementWithLog(stmt, "[ultrawork-db-override] Failed to finalize model update statement", {
			messageId,
		});
	}
	...
	if (variant) {
		...
		try {
			...
			finalizeStatementWithLog(variantStmt, "[ultrawork-db-override] Failed to finalize variant update statement", {
				messageId,
			});
		}
	}
...
async function retryViaMicrotask(
	...
	try {
		...
		if (shouldCloseDb) {
			closeDbWithLog(db, "[ultrawork-db-override] Failed to close DB after deferred DB update", {
				messageId,
				attempt,
			});
		}
