...
import type { ChatMessageHandlerOutput, ChatMessageHooks, ChatMessageInput } from "./types";
...
export function handleRalphLoopMessage(args: {
	...
	if (
		...
		const resumed = isRalphLoopResumeArgument(rawTask) && hooks.ralphLoop.resumeLoop?.(input.sessionID) === true;
		if (!resumed) {
