	...
	return async (input, output): Promise<void> => {
		...
		if (hooks.ralphLoop && sessionID) {
			if (normalizedCommand === "ralph-loop" || normalizedCommand === "ulw-loop") {
				...
				const resumed =
					isRalphLoopResumeArgument(input.arguments || "") && hooks.ralphLoop.resumeLoop?.(sessionID) === true;
				if (!resumed) {
		...
		if (hooks.startWork && normalizedCommand === "start-work" && hasPartsOutput(output)) {
