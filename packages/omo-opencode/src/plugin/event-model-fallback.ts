...
export function createModelFallbackEventHandler(args: {
	...
	const handleAssistantMessageUpdated = async (params: {
		...
		const shouldAutoContinue =
			args.shouldAutoRetrySession(params.sessionID) && !args.isSessionStopped(params.sessionID);
	...
	const handleSessionError = async (params: {
		...
		if (!shouldHandleModelFallback() || !shouldRetryError({ name: params.errorName, message: params.errorMessage }))
			return;
