...
import { getAgentConfigKey, getAgentDisplayName, normalizeAgentForPromptKey } from "../shared/agent-display-names";
import { migrateAgentConfig } from "../shared/permission-compat";
import type { AgentSourceMap, AgentSources } from "./agent-config-types";
import { createProtectedAgentNameSet, filterProtectedAgentOverrides } from "./agent-override-protection";
...
function filterDisabledAgents(
	...
	return Object.fromEntries(Object.entries(agents).filter(([name]) => !disabledAgentNames.has(name.toLowerCase())));
}
...
function filterCustomAgentSources(
	...
	return {
		...
		opencodeGlobalAgents: filterProtectedAgentOverrides(sources.opencodeGlobalAgents, protectedBuiltinAgentNames),
		opencodeProjectAgents: filterProtectedAgentOverrides(sources.opencodeProjectAgents, protectedBuiltinAgentNames),
		pluginAgents: filterProtectedAgentOverrides(sources.pluginAgents, protectedBuiltinAgentNames),
		agentDefinitionAgents: filterProtectedAgentOverrides(sources.agentDefinitionAgents, protectedBuiltinAgentNames),
		opencodeConfigAgents: filterProtectedAgentOverrides(sources.opencodeConfigAgents, protectedBuiltinAgentNames),
	};
}
...
async function createCoreAgentConfig(params: AssembleAgentConfigParams): Promise<Record<string, unknown>> {
...
function applyDefaultAgent(config: Record<string, unknown>, configuredDefaultAgent: string | undefined): void {
...
function assembleSisyphusDisabledConfig(params: AssembleAgentConfigParams): void {
	...
	const filteredConfigAgents = params.sources.configAgent
		? defaultSubagentMode(filterProtectedAgentOverrides(params.sources.configAgent, protectedBuiltinAgentNames))
		: {};
