...
export async function applyAgentConfig(params: ApplyAgentConfigParams): Promise<Record<string, unknown>> {
	...
	const disabledAgentNames = new Set((migratedDisabledAgents ?? []).map((agent: string) => agent.toLowerCase()));
