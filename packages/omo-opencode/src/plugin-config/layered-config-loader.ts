...
function resolveConfigPathAfterLegacyMigration(detectedPath: string): string {
	...
	const canonicalPath = path.join(path.dirname(detectedPath), `${CONFIG_BASENAME}${path.extname(detectedPath)}`);
...
function getUserConfigLayers(): Array<{ readonly configDir: string; readonly configPath: string | null }> {
	...
	return userConfigDirs.map((configDir) => {
		...
		const configPath = detected.format !== "none" ? resolveConfigPathAfterLegacyMigration(detected.path) : null;
...
function resolveUserAgentDefinitions(config: Partial<OhMyOpenCodeConfig>, configDir: string): void {
	if (!config.agent_definitions) return;

	config.agent_definitions = resolveAgentDefinitionPaths(config.agent_definitions, configDir, null);
}

function resolveAncestorAgentDefinitions(config: Partial<OhMyOpenCodeConfig>, ancestorPath: string): void {
	if (!config.agent_definitions) return;

	const ancestorBasePath = path.dirname(ancestorPath);
	const ancestorDir = path.dirname(ancestorBasePath);
	config.agent_definitions = resolveAgentDefinitionPaths(config.agent_definitions, ancestorBasePath, ancestorDir);
}
