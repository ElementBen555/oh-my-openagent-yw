...
import { addConfigLoadError, log, migrateConfigFile, parseJsonc } from "../shared";
...
export function parseConfigPartially(rawConfig: Record<string, unknown>): Partial<OhMyOpenCodeConfig> | null {
...
export function loadConfigFromPath(configPath: string, _ctx: unknown): Partial<OhMyOpenCodeConfig> | null {
	try {
		if (fs.existsSync(configPath)) {
			const content = fs.readFileSync(configPath, "utf-8");
			const rawConfig = parseJsonc<Record<string, unknown>>(content);
