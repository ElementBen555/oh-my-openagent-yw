...
program
	...
	.addOption(
		new Option("--platform <platform>", "Install target platform: opencode, codex, both")
			.choices(["opencode", "codex", "both"])
			.hideHelp(),
	)
...
program
	...
	.addOption(
		new Option("--platform <platform>", "Install target platform: opencode, codex, both").choices([
			"opencode",
			"codex",
			"both",
		]),
...
`,
	...
	.action(async (options) => {
		...
		const doctorOptions: DoctorOptions = {
			mode,
			json: options.json ?? false,
			target: resolveDoctorTarget(process.env.OMO_INVOCATION_NAME),
		};
