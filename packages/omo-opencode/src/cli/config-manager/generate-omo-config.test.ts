...
describe("generateOmoConfig - model fallback system", () => {
	test("uses github-copilot sonnet fallback when only copilot available", () => {
		...
		//#then
		expect(["github-copilot/claude-opus-4.7", "github-copilot/claude-opus-4-7"]).toContain(
	...
	test("uses native OpenAI models when only ChatGPT available", () => {
		...
		//#then
		expect((result.agents as Record<string, { model: string; variant?: string }>).sisyphus.model).toBe(
			"openai/gpt-5.5",
		);
