...
import { type ConfigFormat, detectConfigFormat } from "./opencode-config-format";
...
export async function addPluginToOpenCodeConfig(currentVersion: string): Promise<ConfigMergeResult> {
	...
	const preferredSourceEntry =
		targets.map((target) => findSourcePluginEntryInTarget(target)).find((entry): entry is string => entry !== null) ??
		null;
