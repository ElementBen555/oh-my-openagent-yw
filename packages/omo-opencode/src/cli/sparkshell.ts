...
async function executeSparkShell(
	...
	if (nativeBinaryPath.length > 0) {
		return {
			code: runSpawnedCommand(spawn, nativeBinaryPath, args, { cwd, env }, writeStdout, writeStderr),
			executed: true,
		};
	}

	const appServerClient =
		options.appServerClient === undefined ? createDefaultSparkShellAppServerClient(env) : options.appServerClient;
	if (appServerClient) {
		try {
			...
			writeStderr(
				`[sparkshell] appserver unavailable (${message}); falling back to raw command execution without summary support.\n`,
			);
		}
	}
	...
	return {
		code: runSpawnedCommand(spawn, command, commandArgs, { cwd, env }, writeStdout, writeStderr, transformOutput),
		executed: true,
	};
}
