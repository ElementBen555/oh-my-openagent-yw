...
export function formatDefault(result: DoctorResult): string {
	...
	if (allIssues.length === 0) {
		if (result.target === "codex" && result.codex) {
			...
			lines.push(
				` ${color.green(SYMBOLS.check)} ${color.green(`LazyCodex OK (codex ${codex} · omo ${pluginVer} · ${packageName} ${packageVer})`)}`,
			);
			return lines.join("\n");
		}
		...
		lines.push(
			` ${color.green(SYMBOLS.check)} ${color.green(
				`System OK (opencode ${opencodeVer} · oh-my-opencode ${pluginVer})`.replace("oh-my-opencode", PLUGIN_NAME),
			)}`,
		);
	} else {
