...
export async function checkCodexComponents(deps: CodexComponentsDoctorDeps = {}): Promise<CheckResult> {
	...
	return {
		...
		status,
		message:
			status === "pass" ? "Codex component checks passed" : `${issues.length} Codex component issue(s) detected`,
		details,
		issues,
	};
}
...
function bootstrapDetails(state: BootstrapStateSummary | null, installedVersion: string | null): string[] {
	...
	if (completed === null || completed !== installedVersion) {
		const lines = [
			`${BOOTSTRAP_PENDING_MESSAGE} (installed ${installedLabel}, state ${completed ?? "none"}, lastStatus=${state.lastStatus})`,
		];
...
function degradedDetailLines(entries: readonly BootstrapDegradedEntry[]): string[] {
	return entries.map(
		(entry) =>
			`degraded component=${entry.component} reason=${entry.reason}${entry.hint === undefined ? "" : ` hint=${entry.hint}`}`,
	);
}
