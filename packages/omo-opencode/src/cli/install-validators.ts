...
export function formatConfigSummary(config: InstallConfig): string {
	...
	lines.push(
		formatProvider("MiniMax Coding Plan (minimaxi.com)", config.hasMinimaxCnCodingPlan, "MiniMax-M3 fallback"),
	);
...
export function printBox(content: string, title?: string): void {
	...
	const maxWidth =
		Math.max(...lines.map((line) => line.replace(ANSI_COLOR_PATTERN, "").length), title?.length ?? 0) + 4;
...
function collectCodexOnlyOpenCodeFlagErrors(args: InstallArgs): string[] {
	...
	if (args.minimaxCnCodingPlan !== undefined)
		errors.push("--minimax-cn-coding-plan cannot be used with --platform=codex");
