...
import type * as spawnWithWindowsHideModule from "../../shared/spawn-with-windows-hide";
...
describe("executeOnCompleteHook", () => {
	...
	function createHookDeps(spawnImpl: typeof spawnWithWindowsHideModule.spawnWithWindowsHide = () => createProc(0)) {
	...
	async function importFreshExecuteOnCompleteHook(): Promise<OnCompleteHookModule["executeOnCompleteHook"]> {
	...
	it("command failure logs warning but does not throw", async () => {
		...
		// then
		const warningCall = logCalls.find((call) => call[0] === "On-complete hook exited with non-zero code");
	...
	it("spawn error logs warning but does not throw", async () => {
		...
		// then
		const errorCall = logCalls.find((call) => call[0] === "Failed to execute on-complete hook");
	...
	it("hook stdout and stderr are logged to file logger", async () => {
		...
		// then
		const stdoutCall = logCalls.find((call) => call[0] === "On-complete hook stdout");
		const stderrCall = logCalls.find((call) => call[0] === "On-complete hook stderr");
