...
describe("pollForCompletion", () => {
	...
	it("resets consecutive counter when session becomes busy between checks", async () => {
		...
		unsafeTestValue(ctx.client.session).children = mock(() => Promise.resolve({ data: [] }));
		unsafeTestValue(ctx.client.session).status = mock(() => Promise.resolve({ data: {} }));
	...
	it("returns 1 when session errors while not idle (error not masked by idle gate)", async () => {
		...
		expect(errorCalls.some((call: unknown[]) => String(call[0] ?? "").includes("Session ended with error"))).toBe(
			true,
		);
	});
