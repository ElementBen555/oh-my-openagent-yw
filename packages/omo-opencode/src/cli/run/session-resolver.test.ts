...
const createMockClient = (
	...
	return unsafeTestValue<OpencodeClient>({
		session: {
			get: mock((opts: { path: { id: string } }) => Promise.resolve(getResult ?? { data: { id: opts.path.id } })),
			create: mock(() => {
				const result = createResults[createCallIndex] ?? { data: { id: "new-session-id" } };
				createCallIndex++;
...
describe("resolveSession", () => {
	...
	it("throws error when provided session ID not found", async () => {
		...
		// then
		await Promise.resolve(expect(result).rejects.toThrow(`Session not found: ${sessionId}`));
		expect(mockClient.session.get).toHaveBeenCalledWith({
			path: { id: sessionId },
			query: { directory },
		});
	...
	it("retries session creation on failure", async () => {
		// given
		const mockClient = createMockClient({
			createResults: [{ error: { message: "Network error" } }, { data: { id: "retried-session-id" } }],
		});
	...
	it("throws after all retries exhausted", async () => {
		...
		// then
		await Promise.resolve(expect(result).rejects.toThrow("Failed to create session after all retries"));
	...
	it("session creation returns no ID", async () => {
		// given
		const mockClient = createMockClient({
			createResults: [{ data: undefined }, { data: undefined }, { data: undefined }],
		});
