...
import { createEventState, type EventState, processEvents, serializeError } from "./events";
...
describe("event handling", () => {
	it("does not log verbose event traces by default", async () => {
		...
		try {
			...
			const hasEventTrace = newCalls.some((call) => String(call?.[0] ?? "").includes("custom.event"));
