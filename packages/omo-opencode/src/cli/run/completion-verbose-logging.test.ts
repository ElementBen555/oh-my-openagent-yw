...
const createMockContext = (
	...
	const { childrenBySession = { "test-session": [] }, statuses = {}, verbose = false } = overrides;
...
describe("checkCompletionConditions verbose waiting logs", () => {
	...
	it("prints busy waiting line when verbose is enabled", async () => {
		...
		// then
		expect(result).toBe(false);
		expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining("Waiting: session child-1... is busy"));
	});
});
