...
function getSessionTag(ctx: RunContext, payload: EventPayload): string {
	...
	const sessionID =
		props?.sessionID ?? props?.sessionId ?? info?.sessionID ?? info?.sessionId ?? part?.sessionID ?? part?.sessionId;
...
export function logEventVerbose(ctx: RunContext, payload: EventPayload): void {
	...
	switch (payload.type) {
		...
		case "message.part.delta": {
			...
			console.error(
				pc.dim(`${sessionTag} message.part.delta (${field}): "${preview}${delta.length > 80 ? "..." : ""}"`),
			);
			break;
		}
