...
describe("sparkshell appserver routing", () => {
	unixSocketTest(
		"#given appserver socket #when direct argv runs #then wire client initializes before command exec",
		async () => {
			// given
					...
					message(socket, message) {
						...
						if (request.method === "command/exec" && initialized) {
							socket.send(
								JSON.stringify({
									id: request.id,
									result: { exitCode: 0, stdout: `${request.params.command.join(" ")}\n`, stderr: "" },
								}),
							);
						}
					},
	...
	unixSocketTest(
		"#given unresponsive appserver socket #when direct argv runs #then raw fallback is bounded",
		async () => {
			// given
