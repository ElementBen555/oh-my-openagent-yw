...
function normalizeState(state: Record<string, unknown>): void {
	...
	const sessionOrigins =
		state.session_origins && typeof state.session_origins === "object" && !Array.isArray(state.session_origins)
...
function normalizeSessionFields(target: Record<string, unknown>): void {
	...
	const sessionOrigins =
		target.session_origins && typeof target.session_origins === "object" && !Array.isArray(target.session_origins)
...
export function getWorkByPlanName(
	...
	return (
		getBoulderWorks(state).find((work) => {
