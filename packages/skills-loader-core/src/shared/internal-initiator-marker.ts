...
export function isSyntheticOrInternalTextPart(part: InternalInitiatorTextPartLike): boolean {
...
export function isSyntheticOrInternalUserMessage(message: InternalInitiatorMessageLike): boolean {
...
export function isTerminalNoReplyUserMessage(message: InternalInitiatorMessageLike): boolean {
	const role = message.info?.role ?? message.role;
	if (role !== "user") {
		return false;
	}
...
export function stripInternalInitiatorMarkers(text: string): string {
	return text.replace(INTERNAL_INITIATOR_MARKER_PATTERN, "").replace(INTERNAL_NOREPLY_MARKER_PATTERN, "").trimEnd();
}
...
export function withInternalNoReplyMarker<T extends { type: "text"; text: string }>(part: T): T {
