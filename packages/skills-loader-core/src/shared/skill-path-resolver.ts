...
export function resolveSkillPathReferences(content: string, basePath: string): string {
	...
	return content.replace(/(?<![a-zA-Z0-9="(])@([a-zA-Z0-9_-]+\/[a-zA-Z0-9_.\-/]*)/g, (match, relativePath: string) => {
		...
		if (isPosixAbsolutePath(normalizedBase)) {
			...
			return relativePath.endsWith("/") && !resolvedPath.endsWith("/") ? `${resolvedPath}/` : resolvedPath;
		}
