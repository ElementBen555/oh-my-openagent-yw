...
function getProviderOverride(providerID: string, modelID: string): ModelCapabilityOverride | undefined {
	...
	return GITHUB_COPILOT_GPT5_MODEL.test(normalizeLookupModelID(modelID)) ? GITHUB_COPILOT_GPT5_OVERRIDE : undefined;
}
...
export function getModelCapabilities(input: GetModelCapabilitiesInput): ModelCapabilities {
	...
	const snapshotSource: ModelCapabilitiesDiagnostics["snapshot"]["source"] = runtimeSnapshot?.models?.[
		...
		: bundledSnapshot?.models?.[canonicalization.canonicalModelID]
			? "bundled-snapshot"
			: "none";
	const familySource: ModelCapabilitiesDiagnostics["family"]["source"] = snapshotEntry?.family
		? "snapshot"
		: heuristicFamily?.family
			? "heuristic"
			: "none";
	const variantsSource: ModelCapabilitiesDiagnostics["variants"]["source"] = runtimeVariants
		? "runtime"
	...
	return {
		...
		reasoningEfforts:
			providerOverride?.reasoningEfforts ?? override?.reasoningEfforts ?? heuristicFamily?.reasoningEfforts,
		reasoning: runtimeReasoning ?? snapshotEntry?.reasoning,
		supportsThinking:
			override?.supportsThinking ?? runtimeThinking ?? snapshotEntry?.reasoning ?? heuristicFamily?.supportsThinking,
