...
const AUTO_RETRY_PATTERNS: Array<(combined: string) => boolean> = [
	...
	(combined) =>
		/(?:too\s+many\s+requests|quota\s+will\s+reset\s+after|quota\s*exceeded|exceeded.*quota|usage\s+limit|usage\s*quota|rate\s+limit|limit\s+reached|all\s+credentials\s+for\s+model|cool(?:ing)?\s*down|exhausted\s+your\s+capacity)/i.test(
			combined,
		),
];
