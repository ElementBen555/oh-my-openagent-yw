...
import type { GitBashResolution, RunCommand } from "./types";
...
export const resolveGitBashForCurrentProcess = (
	input: { readonly platform?: string; readonly env?: { readonly [key: string]: string | undefined } } = {},
): GitBashResolution => {
...
export async function prepareGitBashForInstall(input: {
	...
	const resolve =
		input.resolveGitBash ?? (() => resolveGitBashForCurrentProcess({ platform: input.platform, env: input.env }));
