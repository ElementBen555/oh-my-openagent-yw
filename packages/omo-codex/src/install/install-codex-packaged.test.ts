...
test("#given packaged lazycodex tarball layout #when installing Codex plugin #then uses bundled artifacts without source builds", async () => {
	...
	await writeFile(
		...
		JSON.stringify({
			mcpServers: { lsp: { command: "node", args: ["../../lsp-daemon/dist/cli.js", "mcp"], cwd: "." } },
		}),
	);
	...
	const cachedManifest = JSON.parse(await readFile(join(pluginPath, ".codex-plugin", "plugin.json"), "utf8")) as {
		readonly version: string;
	};
	const cachedPackage = JSON.parse(await readFile(join(pluginPath, "package.json"), "utf8")) as {
		readonly version: string;
	};
	const cachedComponentPackage = JSON.parse(
		await readFile(join(pluginPath, "components", "ulw-loop", "package.json"), "utf8"),
	) as { readonly version: string };
	const cachedHooks = JSON.parse(await readFile(join(pluginPath, "hooks", "hooks.json"), "utf8")) as {
		readonly hooks: {
			readonly PostToolUse: readonly [{ readonly hooks: readonly [{ readonly statusMessage: string }] }];
		};
	};
	const cachedComponentHooks = JSON.parse(
		await readFile(join(pluginPath, "components", "ulw-loop", "hooks", "hooks.json"), "utf8"),
	) as {
		readonly hooks: {
			readonly UserPromptSubmit: readonly [{ readonly hooks: readonly [{ readonly statusMessage: string }] }];
		};
	};
	...
	expect(cachedComponentHooks.hooks.UserPromptSubmit[0].hooks[0].statusMessage).toBe(
		"(OmO) Checking Ulw-Loop Steering",
	);
	...
	expect(
		installCommand[2].startsWith(join(codexHome, "plugins", "cache", "sisyphuslabs", "omo", ".tmp-4.5.12-")),
	).toBe(true);
