...
describe("install-codex", () => {
	...
	test(
		...
		async () => {
			...
			expect(configContent).toContain(
				`source = ${formatTomlString(join(codexHome, "plugins", "cache", "sisyphuslabs"))}`,
			);
			...
			const distributionSnapshot = JSON.parse(
				await readFile(join(pluginPath ?? "", "lazycodex-install.json"), "utf8"),
			) as {
			...
			expect(
				(await stat(join(pluginPath ?? "", "components", "ultrawork", "skills", "ulw-plan"))).isDirectory(),
			).toBe(true);
			expect(
				(await stat(join(pluginPath ?? "", "components", "ulw-loop", "skills", "ulw-loop"))).isDirectory(),
			).toBe(true);
			...
			expect(marketplace.plugins).toEqual([
				{ name: "omo", source: { source: "local", path: `./omo/${rootPackage.version}` } },
			]);
	...
	test(
		"#given simulated Windows Codex install #when installing omo #then enables git_bash MCP and trusts shell hooks",
		async () => {
			// given
	...
	test(
		"#given simulated Linux Codex install #when installing omo #then keeps git_bash manifest but disables policy exposure",
		async () => {
			// given
