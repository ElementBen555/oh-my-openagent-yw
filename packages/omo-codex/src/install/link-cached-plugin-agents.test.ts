...
describe("linkCachedPluginAgents", () => {
	test("creates regular file copies on linux so agent roles survive snapshot cleanup", async () => {
		...
		// then
		expect(linked.map((entry) => entry.name).sort()).toEqual(["explorer.toml", "librarian.toml", "planner.toml"]);
	...
	test("returns empty list when plugin has no bundled agents", async () => {
		...
		const manifest = JSON.parse(await readFile(join(pluginRoot, ".installed-agents.json"), "utf8")) as {
			agents: string[];
		};
