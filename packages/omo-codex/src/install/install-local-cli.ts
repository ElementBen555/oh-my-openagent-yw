...
export { formatLazyCodexInstallHelp, PASSTHROUGH_COMMANDS, parseLazyCodexInstallCliArgs } from "./lazycodex-cli-args";
...
export async function runLazyCodexInstallLocalCli(input: {
	...
	if (parsed.kind === "version") {
		const packageJson = JSON.parse(await readFile(join(input.defaultRepoRoot, "package.json"), "utf8")) as {
