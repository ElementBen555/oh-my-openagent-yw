		...
		await transitionRuntimeState(
			...
			(currentRuntimeState) => ({
				...
				members: currentRuntimeState.members.map((member) => {
					if (member.name === "lead")
						return { ...member, sessionId: "ses_alive_lead", status: "running" as const };
					if (member.name === "worker-a")
						return { ...member, sessionId: "ses_dead_a", status: "running" as const };
					if (member.name === "worker-b")
						return { ...member, sessionId: "ses_alive_b", status: "running" as const };
					return member;
				}),
			}),
			config,
