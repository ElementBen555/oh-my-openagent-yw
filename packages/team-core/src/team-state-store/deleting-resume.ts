...
export async function finishDeletingTeam(runtimeState: RuntimeState, config: TeamModeConfig): Promise<boolean> {
...
export async function cleanTerminalTeam(runtimeState: RuntimeState, config: TeamModeConfig): Promise<boolean> {
	return await removeRuntimeDirectory(runtimeState.teamRunId, config);
}
