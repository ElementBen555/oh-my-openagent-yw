...
function isTeamLayoutResultLike(value: unknown): value is TeamLayoutResultLike {
	...
	return (
		typeof value.focusWindowId === "string" &&
...
async function createCallerSession(
	sessionName: string,
): Promise<{ callerSessionId: string; callerPaneId: string; socketPath: string }> {
...
async function invokeRemoveTeamLayout(
	...
	await Promise.resolve(
		Reflect.apply(removeTeamLayout, undefined, [
			teamRunId,
			{
				ownedSession: false,
				targetSessionId,
	...
	test.skipIf(!LIVE)(
		...
		async () => {
			...
			const panesRemoved = await waitForCondition(async () => {
				...
				return (
					panes.success &&
					Object.values(layoutResult.focusPanesByMember).every(
						(paneId) => !panes.stdout.split("\n").includes(paneId),
					)
				);
			});
