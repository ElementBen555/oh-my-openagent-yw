...
**[English](../README.md) | 中文 | [文档](./docs/zh/README.md)**
...
|   📚   | **内置 MCP**                                                    | Both     | Ultimate 运行时注入 Exa（网络搜索）、Context7（官方文档）、Grep.app（GitHub 源码搜索）、Sequential Thinking（动态反思式推理）、GitHub MCP（仓库/PR/Issue 管理）。Light 通过 plugin-scoped MCP 提供 `grep_app`、`context7`、`git_bash`、`lsp`。                                                                              |
...
- **MCP**: 内置 Web 搜索、官方文档直连、GitHub 级代码搜索、动态反思式推理、GitHub 仓库管理。远程 MCP 支持离线自动降级。
